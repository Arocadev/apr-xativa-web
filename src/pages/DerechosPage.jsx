      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        ...
        {loading ? (
            ...
            <table className="w-full text-sm" style={{ fontFamily: 'sans-serif' }}>
              <thead>
                <tr style={{ backgroundColor: '#C0392B' }}>
                  {[t.matricula, t.tipoDerecho, t.tipoAcred, t.fechaInicio, t.fechaFin, t.estado].map((h, i) => (
              ...
              <tbody className="divide-y divide-gray-50">
                {derechos.map((d, i) => (
                  <tr key={d.id} className="hover:bg-gray-50 transition"
                    ...
                    <td className="px-6 py-4 font-mono font-bold text-gray-700">{d.matricula}</td>
                    <td className="px-6 py-4 text-gray-500">{d.tipoDerecho}</td>
                    <td className="px-6 py-4 text-gray-500">{d.tipoAcred}</td>
                    <td className="px-6 py-4 text-gray-500">{d.fechaInicio}</td>
                    <td className="px-6 py-4 text-gray-500">{d.fechaFin}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${
                        d.activo
                          ? 'bg-green-50 text-green-700 border-green-200'
                          : 'bg-red-50 border-red-200'
                      }`} style={!d.activo ? { color: '#C0392B' } : {}}>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
            <p className="text-gray-400 text-sm" style={{ fontFamily: 'sans-serif' }}>{t.sinDerechos}</p>
          </div>
        ) : null}
